...
(function() {
  var Parser, alt, alternatives, name, token;
  ...
  let unwrap = /^function\s*\(\)\s*\{\s*return\s*([\s\S]*);\s*\}/;
  ...
  let o = function(patternString, action, options) {
    var match;
    ...
    let addLocationDataFn = function(first, last) {
      if (!last) {
  ...
  // `Expression`.
  let grammar =  /* The **Root** is the top-level node in the syntax tree. Since we parse bottom-up, */  /* all parsing must end here. */ {
    Root: [
      o('', function() {
        return new Block;
      }), o('Body')
    ...
    Operation: [
      o('UNARY Expression', function() {
        ...
        return new Op('++', $1, null, true);
      }),  /* [The existential operator](http://coffeescript.org/#existential-operator). */ o('Expression ?', function() {
  ...
  //
  //     (2 + 3) * 4
  let operators = [['left', '.', '?.', '::', '?::'], ['left', 'CALL_START', 'CALL_END'], ['nonassoc', '++', '--'], ['left', '?'], ['right', 'UNARY'], ['right', '**'], ['right', 'UNARY_MATH'], ['left', 'MATH'], ['left', '+', '-'], ['left', 'SHIFT'], ['left', 'RELATION'], ['left', 'COMPARE'], ['left', '&'], ['left', '^'], ['left', '|'], ['left', '&&'], ['left', '||'], ['left', 'BIN?'], ['nonassoc', 'INDENT', 'OUTDENT'], ['right', 'YIELD'], ['right', '=', ':', 'COMPOUND_ASSIGN', 'RETURN', 'THROW', 'EXTENDS'], ['right', 'FORIN', 'FOROF', 'FORFROM', 'BY', 'WHEN'], ['right', 'IF', 'ELSE', 'FOR', 'WHILE', 'UNTIL', 'LOOP', 'SUPER', 'CLASS', 'IMPORT', 'EXPORT'], ['left', 'POST_IF'], ['right', 'AWAIT']];
